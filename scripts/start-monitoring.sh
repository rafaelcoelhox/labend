...
# Inclui: Prometheus, Grafana, Alertmanager, Node Exporter, cAdvisor, Jaeger
...
create_directories() {
    ...
    mkdir -p monitoring/grafana/{dashboards,provisioning/datasources,provisioning/dashboards}
...
check_config_files() {
    ...
    local files=(
        ...
        "monitoring/grafana/dashboards/labend-go-runtime.json"
...
wait_for_services() {
    ...
    until curl -s http://localhost:3000/api/health > /dev/null 2>&1; do
        echo -e "${BLUE}Aguardando Grafana...${NC}"
        sleep 2
    done
